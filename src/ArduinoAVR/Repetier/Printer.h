...
public:
  ...
  static INLINE void unsetHomedAll() {
    ...
    flag3 &= ~(PRINTER_FLAG3_X_HOMED | PRINTER_FLAG3_Y_HOMED |
               PRINTER_FLAG3_Z_HOMED);
  }
  ...
  static INLINE void setXHomed(uint8_t b) {
    flag3 =
        (b ? flag3 | PRINTER_FLAG3_X_HOMED : flag3 & ~PRINTER_FLAG3_X_HOMED);
    updateHomedAll();
  }
  ...
  static INLINE void setYHomed(uint8_t b) {
    flag3 =
        (b ? flag3 | PRINTER_FLAG3_Y_HOMED : flag3 & ~PRINTER_FLAG3_Y_HOMED);
    updateHomedAll();
  }
  ...
  static INLINE void setZHomed(uint8_t b) {
    flag3 =
        (b ? flag3 | PRINTER_FLAG3_Z_HOMED : flag3 & ~PRINTER_FLAG3_Z_HOMED);
    updateHomedAll();
  }
  ...
  static INLINE void setPowerOn(uint8_t b) {
    flag1 =
        (b ? flag1 | PRINTER_FLAG1_POWER_ON : flag1 & ~PRINTER_FLAG1_POWER_ON);
  }
  ...
  static INLINE void setPrinting(uint8_t b) {
    flag3 =
        (b ? flag3 | PRINTER_FLAG3_PRINTING : flag3 & ~PRINTER_FLAG3_PRINTING);
  ...
  static INLINE uint8_t isDebugJamOrDisabled() {
    return (flag2 &
            (PRINTER_FLAG2_DEBUG_JAM | PRINTER_FLAG2_JAMCONTROL_DISABLED)) != 0;
  }
...
#define WAIT_UNTIL(condition, timeout)                                         \
  for (uint16_t count = 0;                                                     \
       !condition || count < ((uint16_t)timeout * (uint16_t)1000 /             \
                              (uint16_t)TRINAMIC_WAIT_RESOLUTION_uS);          \
       count++) {                                                              \
